...
flowchart LR
    ...
    subgraph SINKS ["Integrations · LATER"]
        direction TB
        drive["Drive doc"]
        arch["Arch · alt oversight"]
        ledger["SoftLedger / dbt"]
        log["Decision log"]
    end
...
**Read order:** the green core is what runs live. The amber ring is the agent layer the Skill grows into once the rubric is locked. The red ring is what plugs the artifact into the firm's existing stack and the corrections loop that makes it compound. Solid arrows are the live data flow; dotted arrows are deferred wiring.
...
- Why this is the load-bearing slice: framework + schema is where taste shows up. Determinism is a feature — same deck produces same memo, so the analyst can compare the Skill's view against their own on the same fund.

## NEXT — agent layer wrapping the Skill
...
## LATER — corrections loop and integration edges
...
The Skill is what runs live. The roadmap is what we talk to. Together: a deterministic core that does the actual job, plus a system thought that says where it grows. The Skill is the slice; the roadmap is the system.
